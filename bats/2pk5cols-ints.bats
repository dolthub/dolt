...
@test "interact with a multiple primary key table with sql" {
    ...
    [ "$status" -eq 1 ]
    [ "$output" = "column name 'pk2' is non-nullable but attempted to set default value of null" ]
    run dolt sql -q "insert into test (c1,c2,c3,c4,c5) values (6,6,6,6,6)"
    [ "$status" -eq 1 ]
    [ "$output" = "column name 'pk1' is non-nullable but attempted to set default value of null" ]
}