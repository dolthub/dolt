...
@test "create a single primary key table" {
    run dolt table create -s=`nativepath $BATS_TEST_DIRNAME/helper/1pk5col-ints.schema` test
    [ "$status" -eq 0 ]
    [ "$output" = "" ]
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "create a two primary key table" {
    run dolt table create -s=`nativepath $BATS_TEST_DIRNAME/helper/2pk5col-ints.schema` test
    [ "$status" -eq 0 ]
    [ "$output" = "" ]
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "create a table that uses all supported types" {
    run dolt table create -s=`nativepath $BATS_TEST_DIRNAME/helper/1pksupportedtypes.schema` test
    [ "$status" -eq 0 ]
    [ "$output" = "" ]
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "create a table that uses unsupported blob type" {
    run dolt table create -s=`nativepath $BATS_TEST_DIRNAME/helper/1pkunsupportedtypes.schema` test
...
@test "create a repo with two tables" {
    ...
    dolt table create -s=`nativepath $BATS_TEST_DIRNAME/helper/2pk5col-ints.schema` test2
    run dolt ls
    [ "$status" -eq 0 ]
    [[ "$output" =~ "test1" ]] || false
    [[ "$output" =~ "test2" ]] || false
...
@test "create a table with json import" {
    run dolt table import -c -s `nativepath $BATS_TEST_DIRNAME/helper/employees-sch.json` employees `nativepath $BATS_TEST_DIRNAME/helper/employees-tbl.json`
    [ "$status" -eq 0 ]
...
@test "create a table with json import. no schema." {
    run dolt table import -c employees `nativepath $BATS_TEST_DIRNAME/helper/employees-tbl.json`
    [ "$status" -ne 0 ]
    [ "$output" = "Please specify schema file for .json tables." ] 
}

@test "create a table with json import. bad json." {
    run dolt table import -c -s `nativepath $BATS_TEST_DIRNAME/employees-sch.json` employees `nativepath $BATS_TEST_DIRNAME/helper/employees-tbl-bad.json`
    [ "$status" -eq 1 ]
...
@test "create a table with json import. bad schema." {
    run dolt table import -c -s `nativepath $BATS_TEST_DIRNAME/employees-sch-bad.json` employees `nativepath $BATS_TEST_DIRNAME/helper/employees-tbl.json`
    [ "$status" -eq 1 ]
    [[ "$output" =~ "Error creating reader" ]] || false
...
@test "import data from csv and create the table" {
    run dolt table import -c --pk=pk test `nativepath $BATS_TEST_DIRNAME/helper/1pk5col-ints.csv`
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Import completed successfully." ]] || false
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "try to create a table with a bad csv" {
    run dolt table import -c --pk=pk test `nativepath $BATS_TEST_DIRNAME/helper/bad.csv`
    [ "$status" -eq 1 ]
    [[ "$output" =~ "Error creating reader" ]] || false
}

@test "create a table with two primary keys from csv import" {
    run dolt table import -c --pk=pk1,pk2 test `nativepath $BATS_TEST_DIRNAME/helper/2pk5col-ints.csv`
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Import completed successfully." ]] || false
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "import data from psv and create the table" {
    run dolt table import -c --pk=pk test `nativepath $BATS_TEST_DIRNAME/helper/1pk5col-ints.psv`
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Import completed successfully." ]] || false
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "create a table from CSV with common column name patterns" {
    run dolt table import -c --pk=UPPERCASE test `nativepath $BATS_TEST_DIRNAME/helper/caps-column-names.csv`
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Import completed successfully." ]] || false
    run dolt table select test
    [ "$status" -eq 0 ]
...
@test "create a table from excel import with multiple sheets" {
    run dolt table import -c --pk=id employees `nativepath $BATS_TEST_DIRNAME/helper/employees.xlsx`
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Import completed successfully." ]] || false
    run dolt ls
    [ "$status" -eq 0 ]
    ...
    [[ "$output" =~ "tim" ]] || false
    [ "${#lines[@]}" -eq 7 ]
    run dolt table import -c --pk=number basketball `nativepath $BATS_TEST_DIRNAME/helper/employees.xlsx`
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Import completed successfully." ]] || false
    run dolt ls
    [ "$status" -eq 0 ]
...
@test "specify incorrect sheet name on excel import" {
    run dolt table import -c --pk=id bad-sheet-name `nativepath $BATS_TEST_DIRNAME/helper/employees.xlsx`
    [ "$status" -eq 1 ]
...
@test "import an .xlsx file that is not a valid excel spreadsheet" {
    run dolt table import -c --pk=id test `nativepath $BATS_TEST_DIRNAME/helper/bad.xlsx`
    [ "$status" -eq 1 ]
...
@test "create table with sql and dolt table create table. match success/failure" {
    ...
    dolt table create -s=`nativepath $BATS_TEST_DIRNAME/helper/1pk5col-ints.schema` 1pk
    [ "$status" -eq 1 ]
    [ "$output" = "Invalid table name. Table names cannot start with digits." ]
...
@test "import a table with non UTF-8 characters in it" {
    run dolt table import -c --pk=pk test `nativepath $BATS_TEST_DIRNAME/helper/bad-characters.csv`
