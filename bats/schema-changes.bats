...
@test "changing column types should not produce a data diff error" {
    dolt table import -c --pk=pk test `nativebatsdir helper/1pk5col-ints.csv`
    run dolt schema
    [[ "$output" =~ "varchar" ]] || false
    dolt add test
    dolt commit -m "Added test table"
    dolt table import -c -f -pk=pk -s=`nativebatsdir helper/1pk5col-ints.schema` test `nativebatsdir helper/1pk5col-ints.csv`
    run dolt diff
...
@test "change the primary key. view the schema diff" {
    ...
    dolt add test
    dolt commit -m "committed table so we can see diffs"
    dolt table create -f -s=`nativebatsdir helper/1pk5col-ints-diff-pk.schema` test
    run dolt diff --schema
    [ "$status" -eq 0 ]
