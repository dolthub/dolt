...
func newDatabaseCommon(bs types.BatchStore, rt chunks.RootTracker) databaseCommon {
	return databaseCommon{ValueStore: types.NewValueStore(bs), bs: bs, rt: rt, rootRef: rt.Root()}
}

func (db *databaseCommon) MaybeHead(datasetID string) (types.Struct, bool) {
	if r, ok := db.Datasets().MaybeGet(types.NewString(datasetID)); ok {
		return r.(types.Ref).TargetValue(db).(types.Struct), true
	}
	return NewCommit(), false
}

func (db *databaseCommon) Head(datasetID string) types.Struct {
	c, ok := db.MaybeHead(datasetID)
	d.Chk.True(ok, "Database has no Head.")
	return c
}

func (db *databaseCommon) Datasets() types.Map {
	if db.datasets == nil {
		if db.rootRef.IsEmpty() {
			emptyMap := NewMapOfStringToRefOfCommit()
			db.datasets = &emptyMap
		} else {
			db.datasets = db.datasetsFromRef(db.rootRef)
		}
	}
...
func (db *databaseCommon) datasetsFromRef(datasetsRef ref.Ref) *types.Map {
	c := db.ReadValue(datasetsRef).(types.Map)
	return &c
}

func (db *databaseCommon) commit(datasetID string, commit types.Struct) error {
	return db.doCommit(datasetID, commit)
}
...
func (db *databaseCommon) doCommit(datasetID string, commit types.Struct) error {
	currentRootRef, currentDatasets := db.getRootAndDatasets()
	...
	// First commit in database is always fast-foward.
	if !currentRootRef.IsEmpty() {
		...
		if hasHead {
			...
			if !descendsFrom(commit, currentHeadRef, db) {
				return ErrMergeNeeded
			}
		}
	}
	currentDatasets = currentDatasets.Set(types.NewString(datasetID), commitRef)
	return db.tryUpdateRoot(currentDatasets, currentRootRef)
}
...
func (db *databaseCommon) doDelete(datasetID string) error {
	currentRootRef, currentDatasets := db.getRootAndDatasets()
	currentDatasets = currentDatasets.Remove(types.NewString(datasetID))
	return db.tryUpdateRoot(currentDatasets, currentRootRef)
}

func (db *databaseCommon) getRootAndDatasets() (currentRootRef ref.Ref, currentDatasets types.Map) {
	currentRootRef = db.rt.Root()
	currentDatasets = db.Datasets()

	if currentRootRef != currentDatasets.Ref() && !currentRootRef.IsEmpty() {
		// The root has been advanced.
		currentDatasets = *db.datasetsFromRef(currentRootRef)
	}
	return
}

func (db *databaseCommon) tryUpdateRoot(currentDatasets types.Map, currentRootRef ref.Ref) (err error) {
	// TODO: This Commit will be orphaned if the UpdateRoot below fails
	newRootRef := db.WriteValue(currentDatasets).TargetRef()
	// If the root has been updated by another process in the short window since we read it, this call will fail. See issue #404
	if !db.rt.UpdateRoot(newRootRef, currentRootRef) {
