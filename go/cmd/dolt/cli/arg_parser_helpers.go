...
func CreateCommitArgParser(supportsBranchFlag bool) *argparser.ArgParser {
	...
	ap.SupportsString(CommitterParam, "", "committer", "Specify an explicit committer using the standard C O Mmitter {{.LessThan}}committer@example.com{{.GreaterThan}} format.")
	ap.SupportsString(CommitterDateParam, "", "committer-date", "Specify the date used as committer date. If not specified the current system time is used.")
