...
func CreateGlobalArgParser(name string) *argparser.ArgParser {
	...
	ap.SupportsRepeatableString("directory", "C", "dir", "Change to <dir> before executing the subcommand. When repeated, each relative path is resolved from the preceding directory. Equivalent to running dolt from that directory. Matches git -C behaviour.")
