...
func NewSqlEngine(
	...
	return &SqlEngine{
		...
		dsessFactory:   newDoltSession(bcController, pro, mrEnv.Config(), config.Autocommit),
		engine:         engine,
		resultFormat:   format,
	}, nil
}
...
func newDoltSession(bc *branch_control.Controller, pro dsqle.DoltDatabaseProvider, config config.ReadWriteConfig,
	autocommit bool) func(ctx context.Context, mysqlSess *sql.BaseSession, dbs []sql.Database) (*dsess.DoltSession, error) {
