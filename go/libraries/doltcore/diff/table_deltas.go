...
type TableDelta struct {
	...
	FromCustomTable  doltdb.CustomTable
	ToCustomTable    doltdb.CustomTable
...
func GetTableDeltas(ctx context.Context, fromRoot, toRoot doltdb.RootValue) (deltas []TableDelta, err error) {
	...
	err = fromRoot.IterCustomTables(ctx, func(name doltdb.TableName, tbl doltdb.CustomTable) (stop bool, err error) {
		fromDeltas = append(fromDeltas, TableDelta{
			FromName:        name,
			FromCustomTable: tbl,
	...
	err = toRoot.IterCustomTables(ctx, func(name doltdb.TableName, tbl doltdb.CustomTable) (stop bool, err error) {
		toDeltas = append(toDeltas, TableDelta{
			ToName:        name,
			ToCustomTable: tbl,
...
func matchTableDeltas(fromDeltas, toDeltas []TableDelta) (deltas []TableDelta) {
	...
	match := func(t, f TableDelta) TableDelta {
		return TableDelta{
			...
			ToTable:          t.ToTable,
			FromCustomTable:  f.FromCustomTable,
			ToCustomTable:    t.ToCustomTable,
			FromSch:          f.FromSch,
			ToSch:            t.ToSch,
			FromFks:          f.FromFks,
			ToFks:            t.ToFks,
...
func (td TableDelta) IsAdd() bool {
	return (td.FromTable == nil && td.ToTable != nil) || (td.FromCustomTable == nil && td.ToCustomTable != nil)
}
...
func (td TableDelta) HasHashChanged() (bool, error) {
	...
	if td.FromTable != nil || td.ToTable != nil {
		...
		toHash, err = td.ToCustomTable.HashOf(ctx)
		if err != nil {
			return false, err
		}
		fromHash, err = td.FromCustomTable.HashOf(ctx)
		if err != nil {
			return false, err
		}
	}
...
func (td TableDelta) HasSchemaChanged(ctx context.Context) (bool, error) {
	if td.FromCustomTable != nil || td.ToCustomTable != nil {
		return false, nil
	}
...
func (td TableDelta) HasChangesIgnoringColumnTags(ctx context.Context) (bool, error) {
	if td.FromTable == nil && td.ToTable == nil && td.FromCustomTable == nil && td.ToCustomTable == nil {
		return true, nil
	}
	if td.IsAdd() || td.IsDrop() {
		return true, nil
	...
	// If the data hashes have changed, then the table has changed
