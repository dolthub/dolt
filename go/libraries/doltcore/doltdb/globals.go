...
func getGlobalTablesRef(_ context.Context, valDesc val.TupleDesc, valTuple val.Tuple) (result GlobalTablesEntry) {
	result.Db, _ = valDesc.GetString(0, valTuple)
	result.Ref, _ = valDesc.GetString(1, valTuple)
	result.NewTableName, _ = valDesc.GetString(2, valTuple)
	return result
}

func GetGlobalTablePatterns(ctx context.Context, root RootValue, schema string, cb func(string)) error {
	tname := TableName{Name: GlobalTablesTableName, Schema: schema}
	table, found, err := root.GetTable(ctx, tname)
	if err != nil {
		return err
	}
	if !found {
		// dolt_globals doesn't exist, so don't filter any tables.
		return nil
	}

	index, err := table.GetRowData(ctx)
	if table.Format() == types.Format_LD_1 {
		// dolt_globals is not supported for the legacy storage format.
		return nil
	}
	if err != nil {
		return err
