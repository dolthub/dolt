...
type RootObjectDiff interface {
	// Compare returns an integer from -1 to 1, with 0 meaning the diffs are equal to each other. This only compares the
	// unique identifier for the diff, so different diffs may return that they are equal.
	Compare(ctx context.Context, other RootObjectDiff) (int, error)
