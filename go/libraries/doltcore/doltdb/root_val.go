...
func (root *RootValue) StashConflicts(ctx context.Context) (*RootValue, *ConflictStash, error) {
	names, err := root.GetTableNames(ctx)
	if err != nil {
		return nil, nil, err
	}
...
func (root *RootValue) StashViolations(ctx context.Context) (*RootValue, *ViolationStash, error) {
	names, err := root.GetTableNames(ctx)
	if err != nil {
		return nil, nil, err
	}
...
// data.
func (root *RootValue) ApplyConflictStash(ctx context.Context, stash map[string]*ConflictData) (*RootValue, error) {
	updatedRoot := root
...
func getConflictData(ctx context.Context, tbl *Table) (*ConflictData, error) {
...
func setConflictData(ctx context.Context, tbl *Table, data *ConflictData) (*Table, error) {
	var err error
	if !data.HasConflicts {
		tbl, err = tbl.ClearConflicts(ctx)
	} else {
