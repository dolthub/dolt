...
func CreateIndexForTable(
	...
	if !IsValidTableName(indexName) {
		return nil, fmt.Errorf("invalid index name `%s` as they must match the regular expression %s", indexName, TableNameRegexStr)
	}
	...
	if replacingIndex { // verify that the pre-existing index data is valid
		...
		indexRowData, err := RebuildIndex(ctx, newTable, index.Name())
		if err != nil {
			return nil, err
		}
