...
func Backup(ctx context.Context, srcDB *doltdb.DoltDB, tempTableDir string, backup env.Remote, ds datas.Dataset) error {
	destDB, err := backup.GetRemoteDB(ctx, srcDB.ValueReadWriter().Format())

	// ds.head.Name() = tag, or commit
	// ds.Id() = target ref?
	//ref := ds.ID()
	...
	rf, err := ref.Parse(headRef.String())
	if err != nil {
		return err
	}
	var stRef types.Ref
	...
	case ref.BranchRef:
		...
		cs, err := doltdb.NewCommitSpec(headRef.String())
		if err != nil {
			return err
		}
		commit, err := srcDB.Resolve(ctx, cs, nil)
