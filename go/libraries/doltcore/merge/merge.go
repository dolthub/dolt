...
func MergeCommits(ctx context.Context, commit, mergeCommit *doltdb.Commit, opts editor.Options) (*doltdb.RootValue, map[string]*MergeStats, error) {
	...
	return MergeRoots(ctx, ourRoot, theirRoot, ancRoot, opts, MergeOpts{IsCherryPick: false})
}
...
func MergeRoots(ctx context.Context, ourRoot, theirRoot, ancRoot *doltdb.RootValue, opts editor.Options, mergeOpts MergeOpts) (*doltdb.RootValue, map[string]*MergeStats, error) {
	...
	merger, err := NewMerger(ourRoot, theirRoot, ancRoot, ourRoot.VRW(), ourRoot.NodeStore())
	if err != nil {
		return nil, nil, err
	}
