...
const (
	...
	CustomTypeIdentifier             Identifier = "custom"
)
...
func FromSqlType(sqlType sql.Type) (TypeInfo, error) {
	if customType, ok := sqlType.(gmstypes.Custom); ok {
		return CreateCustomTypeFromSqlType(customType), nil
	}
...
func FromTypeParams(id Identifier, params map[string]string) (TypeInfo, error) {
	...
	case CustomTypeIdentifier:
		return CreateCustomTypeFromParams(params)
...
func FromKind(kind types.NomsKind) TypeInfo {
	...
	case types.CustomInlineKind:
		//TODO: this wouldn't be static like this
		c, ok := gmstypes.DeserializeCustomType(0)
		if !ok {
			panic("no custom types have been registered")
		}
		return &customType{c}
