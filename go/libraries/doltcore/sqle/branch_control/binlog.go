...
func (binlog *Binlog) Serialize() []byte {
	...
	buffer.WriteByte(byte(currentBinlogVersion >> 8))
	buffer.WriteByte(byte(currentBinlogVersion))
	// Write the number of entries
	binlogSize := uint64(len(binlog.rows))
	buffer.WriteByte(byte(binlogSize >> 56))
	buffer.WriteByte(byte(binlogSize >> 48))
	buffer.WriteByte(byte(binlogSize >> 40))
	buffer.WriteByte(byte(binlogSize >> 32))
	buffer.WriteByte(byte(binlogSize >> 24))
	buffer.WriteByte(byte(binlogSize >> 16))
	buffer.WriteByte(byte(binlogSize >> 8))
	buffer.WriteByte(byte(binlogSize))
	// Write the rows
...
func (row *BinlogRow) Serialize(buffer *bytes.Buffer) {
	...
	buffer.WriteByte(byte(branchLen >> 8))
	buffer.WriteByte(byte(branchLen))
	buffer.WriteString(row.Branch)
	// Write the user
	userLen := uint16(len(row.User))
	buffer.WriteByte(byte(userLen >> 8))
	buffer.WriteByte(byte(userLen))
	buffer.WriteString(row.User)
	// Write the host
	hostLen := uint16(len(row.Host))
	buffer.WriteByte(byte(hostLen >> 8))
	buffer.WriteByte(byte(hostLen))
	buffer.WriteString(row.Host)
	// Write the permissions
	buffer.WriteByte(byte(row.Permissions >> 56))
	buffer.WriteByte(byte(row.Permissions >> 48))
	buffer.WriteByte(byte(row.Permissions >> 40))
	buffer.WriteByte(byte(row.Permissions >> 32))
	buffer.WriteByte(byte(row.Permissions >> 24))
	buffer.WriteByte(byte(row.Permissions >> 16))
	buffer.WriteByte(byte(row.Permissions >> 8))
	buffer.WriteByte(byte(row.Permissions))
}
