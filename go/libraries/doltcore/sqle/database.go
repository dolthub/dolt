...
func (db Database) getTableInsensitive(ctx *sql.Context, head *doltdb.Commit, ds *dsess.DoltSession, root *doltdb.RootValue, tblName string) (sql.Table, bool, error) {
	...
	case branch_control.AccessTableName:
		dt, found = branch_control.StaticController.Access, true
	case branch_control.NamespaceTableName:
		dt, found = branch_control.StaticController.Namespace, true
	case branch_control.AccessBinlogTableName:
		dt, found = branch_control.StaticController.Access.BinlogTable(), true
	case branch_control.NamespaceBinlogTableName:
		dt, found = branch_control.StaticController.Namespace.BinlogTable(), true
	}
	if found {
		return dt, found, nil
	}
