...
func (iw *docsWriter) StatementBegin(ctx *sql.Context) {
	...
	if !found {
		...
		newRootValue, err := roots.Working.CreateEmptyTable(ctx, doltdb.TableName{Name: doltdb.DocTableName}, newSchema)
		if err != nil {
			iw.errDuringStatementBegin = err
			return
		}
	...
	tableWriter, err := dbState.WriteSession().GetTableWriter(ctx, doltdb.TableName{Name: doltdb.DocTableName}, dbName, dSess.SetRoot)
	if err != nil {
		iw.errDuringStatementBegin = err
		return
	}
