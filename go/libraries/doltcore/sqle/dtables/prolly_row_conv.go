...
func (c ProllyRowConverter) putFields(ctx context.Context, tup val.Tuple, proj val.OrdinalMapping, desc val.TupleDesc, targetTypes []sql.Type, dstRow sql.UntypedSqlRow) error {
	for i, j := range proj {
		if j == -1 {
			continue
		}
