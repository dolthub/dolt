	...
	var diffTableFunctionTests = []enginetest.ScriptTest{
		{
			...
			SetUpScript: []string{
				...
				//select @Commit0, @Commit1, @Commit2, @Commit3, @Commit4;
				//select * from dolt_diff("t", @Commit1, @Commit2);
				//select * from dolt_diff("t", @Commit2, @Commit3);
				//select * from dolt_diff("t", @Commit3, @Commit4);
				//select * from dolt_diff("t", @Commit1, @Commit4);
			},
			Assertions: []enginetest.ScriptTestAssertion{
				{
					Query:    "SELECT COUNT(*) from dolt_diff('t', @Commit1, @Commit2);",
					Expected: []sql.Row{{1}},
				},
				{
				...
				{
					Query:    "SELECT COUNT(*) from dolt_diff('t', @Commit3, @Commit4);",
					Expected: []sql.Row{{3}},
				},
				{
					Query:    "SELECT COUNT(*) from dolt_diff('t', @Commit1, @Commit4);",
					Expected: []sql.Row{{4}},
				},
				{
					// TODO: The analyzer doesn't currently understand the schema returned from
					//       the table function, so we can't select column names yet
					Query:    "SELECT to_pk from dolt_diff('t', @Commit1, @Commit2);",
					Expected: []sql.Row{{1}},
				},
				// TODO: Add error case tests for invalid commits
				//       - fromCommit is not an ancestor of toCommit
				//       - non-ref test
				// TODO: Add tests for branch/ref support
				//       Add tests for datetime support ?
			},
		},
		{
		...
		//	Query:    "SELECT * from dolt_diff(abs(-1), concat('j', 'a', 's', 'o', 'n'));",
		//	Expected: []sql.Row{{1, 1, "jason"}},
		//},
	}
