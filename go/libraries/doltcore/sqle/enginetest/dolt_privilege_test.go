...
var DoltOnlyRevisionTableFunctionPrivilegeTests = []queries.UserPrivilegeTest{
	...
	},
	{
		...
		SetUpScript: []string{
			...
			"use otherdb;",
			"CREATE TABLE other_table (pk BIGINT PRIMARY KEY, col1 varchar(20));",
			"INSERT INTO other_table VALUES (1, 'other1'), (2, 'other2');",
			"CREATE TABLE other_table2 (pk BIGINT PRIMARY KEY, col1 varchar(20));",
			"INSERT INTO other_table2 VALUES (1, 'other3'), (2, 'other4');",
			"CREATE USER tester@localhost;",
			"use mydb;",
			"call dolt_branch('b1');",
			"use mydb/b1;",
		...
		Assertions: []queries.UserPrivilegeTestAssertion{
			...
			},
			{
				User:        "tester",
				Host:        "localhost",
				Query:       "SELECT * FROM dolt_query_diff('SELECT pk FROM otherdb.other_table', 'SELECT pk FROM otherdb.other_table');",
				ExpectedErr: sql.ErrDatabaseAccessDeniedForUser,
			},
			{
				User:  "tester",
			...
			},
			{
				User:        "tester",
				Host:        "localhost",
				Query:       "SELECT * FROM dolt_query_diff('SELECT t.pk FROM mydb.test t JOIN mydb.test2 t2 ON t.pk = t2.pk', 'SELECT t.pk FROM mydb.test t JOIN mydb.test2 t2 ON t.pk = 2');",
				ExpectedErr: sql.ErrTableAccessDeniedForUser,
			},
			{
				User:     "root",
			...
			},
			{
				User:  "tester",
				Host:  "localhost",
				Query: "SELECT * FROM dolt_query_diff('SELECT pk, col1 FROM otherdb.other_table', 'SELECT pk, col1 FROM mydb.test');",
				Expected: []sql.Row{
					{1, "other1", nil, nil, "deleted"},
					{2, "other2", nil, nil, "deleted"},
					{nil, nil, 1, "first row", "added"},
					{nil, nil, 2, "second row", "added"},
				},
			},
			{
				User:  "tester",
			...
			},
			{
				User:  "tester",
				Host:  "localhost",
				Query: "EXPLAIN FORMAT=TREE SELECT * FROM dolt_query_diff('SELECT pk, JSON_EXTRACT(col1, ''$.a'') FROM mydb.test', 'SELECT pk, JSON_EXTRACT(col1, ''$.a'') FROM mydb.test');",
				// EXPLAIN plans the query but does not execute.
				// The subquery uses JSON_EXTRACT on non-JSON text
				// which would fail at runtime.
				Expected: []sql.Row{
					{"Project"},
					{" ├─ columns: [dolt_query_diff.from_pk, dolt_query_diff.from_JSON_EXTRACT(col1, '$.a'), dolt_query_diff.to_pk, dolt_query_diff.to_JSON_EXTRACT(col1, '$.a'), dolt_query_diff.diff_type]"},
					{" └─ TableAlias(dolt_query_diff)"},
					{"     └─ DOLT_QUERY_DIFF(''SELECT pk, JSON_EXTRACT(col1, ''$.a'') FROM mydb.test'', ''SELECT pk, JSON_EXTRACT(col1, ''$.a'') FROM mydb.test'')"},
				},
			},
			{
				User:        "tester",
				Host:        "localhost",
				Query:       "SELECT * FROM dolt_query_diff('SELECT pk, JSON_EXTRACT(col1, ''$.a'') FROM mydb.test', 'SELECT pk, JSON_EXTRACT(col1, ''$.a'') FROM mydb.test');",
				ExpectedErr: sql.ErrInvalidJSONText,
			},
		},
	},
	{
		Name: "dolt_query_diff view privilege checking",
		SetUpScript: []string{
			"CREATE TABLE test (pk BIGINT PRIMARY KEY, col1 varchar(20));",
			"INSERT INTO test VALUES (1, 'first row'), (2, 'second row');",
			"CREATE VIEW v_test AS SELECT pk, col1 FROM test;",
			"call dolt_commit('-Am', 'first commit');",
			"CREATE USER tester@localhost;",
			"call dolt_branch('b1');",
			"use mydb/b1;",
			"GRANT SELECT ON mydb.v_test TO tester@localhost;",
		},
		Assertions: []queries.UserPrivilegeTestAssertion{
			{
				User:        "tester",
				Host:        "localhost",
				Query:       "SELECT * FROM dolt_query_diff('SELECT pk FROM v_test', 'SELECT pk FROM v_test');",
				ExpectedErr: sql.ErrPrivilegeCheckFailed,
			},
			{
				User:     "root",
			...
			},
			{
				User:     "root",
				Host:     "localhost",
				Query:    "REVOKE SELECT ON mydb.v_test FROM tester@localhost;",
				Expected: []sql.Row{{types.NewOkResult(0)}},
			},
			{
				User:        "tester",
