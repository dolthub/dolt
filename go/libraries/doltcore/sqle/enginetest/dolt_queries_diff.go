...
var PatchTableFunctionScriptTests = []queries.ScriptTest{
	...
	},
	{
		...
		Assertions: []queries.ScriptTestAssertion{
			...
			{
				Query: "describe SELECT /*+ JOIN_ORDER(diff_type_name,dolt_patch) LOOKUP_JOIN(diff_type_name,dolt_patch) */ * from diff_type_name join dolt_patch(@Commit0, @Commit0, 't') on diff_type_name.t = diff_type;",
				Expected: []sql.Row{
					{"LookupJoin"},
					{" ├─ (diff_type_name.t = dolt_patch.diff_type)"},
					{" ├─ Exchange"},
					{" │   └─ Table"},
					{" │       ├─ name: diff_type_name"},
					{" │       └─ columns: [name t]"},
			...
			{
				Query: "describe SELECT ( SELECT statement FROM (SELECT * FROM dolt_patch(@Commit0, @Commit0, 't') where diff_type = diff_type_name.t) as rhs) from diff_type_name;",
				Expected: []sql.Row{
					{"Project"},
					{" ├─ columns: [Subquery"},
					{" │   ├─ cacheable: false"},
					{" │   └─ Project"},
					{" │       ├─ columns: [rhs.statement]"},
					{" │       └─ SubqueryAlias"},
					{" │           ├─ name: rhs"},
					{" │           ├─ outerVisibility: true"},
					{" │           ├─ cacheable: false"},
					{" │           └─ Project"},
					{" │               ├─ columns: [dolt_patch.diff_type, dolt_patch.statement]"},
					{" │               └─ Filter"},
					{" │                   ├─ (dolt_patch.diff_type = diff_type_name.t)"},
					{" │                   └─ TableAlias(dolt_patch)"},
					{" │                       └─ IndexedTableAccess(DOLT_PATCH(@Commit0, @Commit0, 't'))"},
					{" │                           └─ index: [DOLT_PATCH(@Commit0, @Commit0, 't').diff_type]"},
					{" │  ]"},
