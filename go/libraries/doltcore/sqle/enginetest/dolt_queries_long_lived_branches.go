...
var DoltLongLivedBranchTests = []queries.ScriptTest{
	...
	{
		Name: "Test re-resolve with multiple branches and conflicts",
		SetUpScript: []string{
			"CREATE TABLE test (pk INT PRIMARY KEY, val INT);",
			"INSERT INTO test VALUES (1, 1), (2, 2), (3, 3);",
			"CALL DOLT_COMMIT('-Am', 'Seed commit');",
			"CALL DOLT_BRANCH('branchA');",
			"CALL DOLT_BRANCH('branchB');",
			"CALL DOLT_BRANCH('branchC');",
			"CALL DOLT_CHECKOUT('branchA');",
			"INSERT INTO test VALUES (4, 4);",
			"CALL DOLT_COMMIT('-Am', 'Insert (4,4) on branchA');",
			"CALL DOLT_CHECKOUT('branchB');",
			"INSERT INTO test VALUES (5, 5);",
			"CALL DOLT_COMMIT('-Am', 'Insert (5,5) on branchB');",
			"CALL DOLT_CHECKOUT('branchC');",
			"INSERT INTO test VALUES (6, 6);",
			"CALL DOLT_COMMIT('-Am', 'Insert (6,6) on branchC');",
			"CALL DOLT_CHECKOUT('branchA');",
			"UPDATE test SET val = 10 WHERE pk = 1;",
			"CALL DOLT_COMMIT('-Am', 'Update (1,10) on branchA');",
			"CALL DOLT_CHECKOUT('branchB');",
			"UPDATE test SET val = 20 WHERE pk = 1;",
			"CALL DOLT_COMMIT('-Am', 'Update (1,20) on branchB');",
			"CALL DOLT_CHECKOUT('branchC');",
			"CALL DOLT_MERGE('branchB');",
			"CALL DOLT_CHECKOUT('branchB');",
			"SET @@autocommit = 0;",
		},
		Assertions: []queries.ScriptTestAssertion{
			{
				Query:    "CALL DOLT_MERGE('branchA');",
				Expected: []sql.Row{{"", 0, 1, "conflicts found"}},
			},
			{
				Query:    "SELECT base_pk, our_val, their_val FROM dolt_conflicts_test;",
				Expected: []sql.Row{{1, 20, 10}},
			},
			{
				Query:    "UPDATE dolt_conflicts_test SET our_val = 30;",
			...
			{
				Query:            "CALL DOLT_CHECKOUT('branchC');",
				SkipResultsCheck: true,
			},
			{
				Query:            "CALL DOLT_MERGE('branchB');",
				SkipResultsCheck: true,
			},
			{
