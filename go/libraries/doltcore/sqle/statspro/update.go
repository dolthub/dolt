...
func createNewStatsBuckets(ctx *sql.Context, sqlTable sql.Table, dTab *doltdb.Table, indexes []sql.Index, idxMetas []indexMeta) (map[sql.StatQualifier]*DoltStats, error) {
	...
	for _, meta := range idxMetas {
		...
		ret[updater.qual].Statistic.DistinctCnt = float64(updater.globalDistinct)
		ret[updater.qual].Statistic.RowCnt = float64(updater.globalCount)
...
func (u *bucketBuilder) finalize(ctx context.Context, ns tree.NodeStore) (DoltBucket, error) {
	...
	u.mcvs.Sort()
	u.mcvs.Truncate(2 * u.count / u.distinct) // only keep MCVs that are > twice as common as average
	...
	return DoltBucket{
		Bucket: &stats.Bucket{
			RowCnt:      u.count,
			DistinctCnt: u.distinct,
			BoundCnt:    u.currentCnt,
			McvVals:     mcvRows,
...
func (m mcvHeap) Counts() []float64 {
	ret := make([]float64, len(m))
	for i, mcv := range m {
		ret[i] = float64(mcv.cnt)
	}
	return ret
}
