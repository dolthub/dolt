...
func (t *AlterableDoltTable) CreateForeignKey(
	...
	if fkName != "" && !doltdb.IsValidIdentifier(fkName) {
		return fmt.Errorf("invalid foreign key name `%s` as it must match the regular expression %s", fkName, doltdb.UnquotedIdentifierRegexStr)
	}
...
func createIndexForTable(
	...
	if !doltdb.IsValidIdentifier(indexName) {
		return nil, fmt.Errorf("invalid index name `%s` as they must match the regular expression %s", indexName, doltdb.UnquotedIdentifierRegexStr)
	}
