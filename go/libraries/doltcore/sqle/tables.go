...
func (t *WritableDoltTable) Truncate(ctx *sql.Context) (int, error) {
	...
	newTable, err = doltdb.RebuildAllIndexes(ctx, newTable)
	if err != nil {
		return 0, err
	}
...
func (t *AlterableDoltTable) CreateIndex(ctx *sql.Context, indexName string, using sql.IndexUsing, constraint sql.IndexConstraint, columns []sql.IndexColumn, comment string) error {
	...
	ret, err := doltdb.CreateIndexForTable(ctx, t.table, indexName, using, constraint, columns, true, comment)
	if err != nil {
		return err
	}
	root, err := t.db.GetRoot(ctx)
...
func (t *AlterableDoltTable) CreateForeignKey(
	...
	if !ok {
		...
		ret, err := doltdb.CreateIndexForTable(ctx, t.table, "", sql.IndexUsing_Default, sql.IndexConstraint_None, sqlColNames, false, "")
		if err != nil {
			return err
		}
	...
	if !ok {
		...
		if parentPKs.ContainsAll(refColTags) {
			...
			ret, err := doltdb.CreateIndexForTable(ctx, refTbl, "", sql.IndexUsing_Default, sql.IndexConstraint_None, colNames, false, "")
			if err != nil {
				return err
			}
			refTbl = ret.NewTable
