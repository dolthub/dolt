...
func (jr JsonRows) decodeJSONRows(format *types.Format, sch schema.Schema) ([]row.Row, error) {
	rowMaps := jr.Rows
	numRows := len(rowMaps)
	...
	for i := 0; i < numRows; i++ {
		rows[i], err = convToRow(format, sch, rowMaps[i])
		if err != nil {
			return nil, err
		}
	}
...
func convToRow(format *types.Format, sch schema.Schema, rowMap map[string]interface{}) (row.Row, error) {
	...
	return row.New(format, sch, taggedVals), nil
}
