...
type ReadStats struct {
	...
	Percent float64 // -1 when the total size is unknown or zero.
}
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				...
				// HTTP ContentLength is -1 when unknown (e.g. a chunked response).
				// Keep that sentinel for unknown sizes; zero also has no meaningful percentage.
				percent := float64(-1)
				if rws.size > 0 {
