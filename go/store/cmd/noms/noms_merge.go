...
func runMerge(ctx context.Context, args []string) int {
	...
	_, err = db.SetHead(ctx, outDS, db.WriteValue(ctx, datas.NewCommit(merged, types.NewSet(ctx, types.Format_7_18, db, leftDS.HeadRef(), rightDS.HeadRef()), types.EmptyStruct)))
	d.PanicIfError(err)
	if !verbose.Quiet() {
		status.Printf("Done")
		status.Done()
