...
type Database interface {
	...
	// UpdateWorkspaceValue updates the dataset given, setting its value to a new workspace object with the ref and
	// meta given. If the dataset given already had a value, it must match the hash given or this method returns
	// ErrOptimisticLockFailed and the caller must retry.
	// The returned Dataset is always the newest snapshot, regardless of
	// success or failure, and Datasets() is updated to match backing storage
	// upon return as well.
	UpdateWorkspaceValue(ctx context.Context, ds Dataset, ref types.Ref, meta WorkspaceMeta, prevHash hash.Hash) (Dataset, error)
