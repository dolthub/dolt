...
func TestPatchPathPartCompare(t *testing.T) {
	...
	for i, tc := range testCases {
		res01, err := pathPartCompare(types.Format_Default, tc[0], tc[1])
		require.NoError(t, err)
		res00, err := pathPartCompare(types.Format_Default, tc[0], tc[0])
		require.NoError(t, err)
		res10, err := pathPartCompare(types.Format_Default, tc[1], tc[0])
		require.NoError(t, err)
...
func TestPatchPathIsLess(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		zeroLTOne, err := pathIsLess(types.Format_Default, p0, p1)
		require.NoError(t, err)
		zeroLTZero, err := pathIsLess(types.Format_Default, p0, p0)
		require.NoError(t, err)
		oneLTZero, err := pathIsLess(types.Format_Default, p1, p0)
		require.NoError(t, err)
...
func TestPatchSort(t *testing.T) {
	...
	types.SortWithErroringLess(PatchSort{shuffledPaths, types.Format_Default})
