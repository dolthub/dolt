...
type (
	printFunc func(ctx context.Context, format *types.Format, w io.Writer, op prefixOp, key, val types.Value) error
)
...
func PrintDiff(ctx context.Context, format *types.Format, w io.Writer, v1, v2 types.Value, leftRight bool) (err error) {
	...
	if !ShouldDescend(v1, v2) {
		line(ctx, format, w, DEL, nil, v1)
		return line(ctx, format, w, ADD, nil, v2)
	}
	...
	// element in the Path
	go func() {
		Diff(ctx, format, v1, v2, dChan, stopChan, leftRight, nil)
		close(dChan)
	}()
	...
	for d := range dChan {
		...
		if parentPathChanged || firstTime {
			firstTime = false
			err = writeHeader(w, format, parentPath, &wroteHdr)
		}

		lastPart := d.Path[len(d.Path)-1]
		parentEl := parentPath.Resolve(ctx, format, v1, nil)
		...
		case types.Map:
			if indexPath, ok := lastPart.(types.IndexPath); ok {
				...
				hip1 := hip
				hip1.IntoKey = true
				key = hip1.Resolve(ctx, format, parent, nil)
			} else {
		...
		if d.OldValue != nil {
			err = pfunc(ctx, format, w, DEL, key, d.OldValue)
		}
		if d.NewValue != nil {
			err = pfunc(ctx, format, w, ADD, key, d.NewValue)
		}
		if err != nil {
			stopDiff()
			break
...
func writeHeader(w io.Writer, format *types.Format, p types.Path, wroteHdr *bool) error {
	if *wroteHdr {
		return nil
	}
	*wroteHdr = true
...
func line(ctx context.Context, format *types.Format, w io.Writer, op prefixOp, key, val types.Value) error {
	genPrefix := func(w *writers.PrefixWriter) []byte {
		return []byte(op)
	}
	pw := &writers.PrefixWriter{Dest: w, PrefixFunc: genPrefix, NeedsPrefix: true}
	if key != nil {
		writeEncodedValue(ctx, format, pw, key)
		write(w, []byte(": "))
	}
	writeEncodedValue(ctx, format, pw, val)
	return write(w, []byte("\n"))
}

func field(ctx context.Context, format *types.Format, w io.Writer, op prefixOp, name, val types.Value) error {
	genPrefix := func(w *writers.PrefixWriter) []byte {
		return []byte(op)
	}
	pw := &writers.PrefixWriter{Dest: w, PrefixFunc: genPrefix, NeedsPrefix: true}
