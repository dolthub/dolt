...
func (m *merger) threeWay(ctx context.Context, format *types.Format, a, b, parent types.Value, path types.Path) (merged types.Value, err error) {
	...
	pDescription := "<nil>"
	if parent != nil {
		pDescription = types.TypeOf(parent).Describe(ctx, format)
	}
	return parent, newMergeConflict("Cannot merge %s and %s on top of %s.", types.TypeOf(a).Describe(ctx, format), types.TypeOf(b).Describe(ctx, format), pDescription)
}
...
func (m *merger) threeWayStructMerge(ctx context.Context, format *types.Format, a, b, parent types.Struct, path types.Path) (merged types.Value, err error) {
	apply := func(target candidate, change types.ValueChanged, newVal types.Value) candidate {
		...
		panic(fmt.Errorf("bad key type in diff: %s", types.TypeOf(change.Key).Describe(ctx, format)))
	}
...
func mapAssert(ctx context.Context, vrw types.ValueReadWriter, a, b, parent types.Value) (aMap, bMap, pMap types.Map, ok bool) {
	...
	if parent != nil {
		...
		pMap, pOk = types.NewMap(ctx, vrw.Format(), vrw), true
	}
