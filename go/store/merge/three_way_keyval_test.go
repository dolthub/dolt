...
func (s *ThreeWayStructMergeSuite) SetupSuite() {
	s.create = func(seq seq) (val types.Value) {
		if seq != nil {
			...
			val = types.NewStruct(types.Format_7_18, "TestStruct", fields)
		}
		return
	}
	s.typeStr = "Struct"
...
func (s *ThreeWayKeyValMergeSuite) TestThreeWayMerge_NestedConflict() {
	...
	s.tryThreeWayConflict(s.create(a), s.create(mm2b), s.create(mm2), types.EncodedValue(context.Background(), types.Format_7_18, types.NewSet(context.Background(), types.Format_7_18, s.vs)))
	s.tryThreeWayConflict(s.create(a), s.create(mm2b), s.create(mm2), types.EncodedValue(context.Background(), types.Format_7_18, s.create(aa1b)))
}
