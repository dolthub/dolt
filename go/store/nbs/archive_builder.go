...
func RunExperiment(cs chunks.ChunkStore, dagGroups *ChunkRelations, cfg ExpConfig, p PrintfFunc) (err error) {
	if gs, ok := cs.(*GenerationalNBS); ok {
		...
		for tf, ogcs := range oldgen {
			...
			if !cfg.Cmp || !cfg.Group {
				err = copyAllChunks(ogcs, idx, outPath, cfg, p)
			} else {
				err = groupAllChunks(ogcs, idx, dagGroups, outPath, cfg, p)
			}
...
func groupAllChunks(cs chunkSource, idx tableIndex, dagGroups *ChunkRelations, archivePath string, cfg ExpConfig, p PrintfFunc) error {
...
func copyAllChunks(cs chunkSource, idx tableIndex, archivePath string, cfg ExpConfig, p PrintfFunc) error {
	records := make([]getRecord, idx.chunkCount())
	for i := uint32(0); i < idx.chunkCount(); i++ {
		var h hash.Hash
		_, err := idx.indexEntry(i, &h)
...
func (cg *chunkGroup) addChunk(c *chunks.Chunk, cfg ExpConfig, p PrintfFunc) {
...
func (cg *chunkGroup) recalculate(cfg ExpConfig, p PrintfFunc) {
...
func (cr *ChunkRelations) convertToChunkGroups(chks map[hash.Hash]*chunks.Chunk, cfg ExpConfig, p PrintfFunc) []*chunkGroup {
	...
	for _, v := range cr.groups() {
		...
		result = append(result, newChunkGroup(c, cfg, p))
	}
	return result
}
