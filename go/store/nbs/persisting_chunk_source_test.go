...
func TestPersistingChunkStoreEmpty(t *testing.T) {
	...
	ccs := newPersistingChunkSource(context.Background(), mt, nil, newFakeTablePersister(&UnlimitedQuotaProvider{}), make(chan struct{}, 1), &Stats{})
...
func TestPersistingChunkStore(t *testing.T) {
	...
	ccs := newPersistingChunkSource(context.Background(), mt, nil, pausingFakeTablePersister{newFakeTablePersister(&UnlimitedQuotaProvider{}), trigger}, make(chan struct{}, 1), &Stats{})
