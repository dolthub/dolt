...
func TestChunkStoreCommit(t *testing.T) {
	...
	err = store.Put(context.Background(), newRootChunk, mockGetAddrs)
	require.NoError(t, err)
	...
	err = store.Put(context.Background(), secondRootChunk, mockGetAddrs)
	require.NoError(t, err)
...
func TestChunkStoreManifestPreemptiveOptimisticLockFail(t *testing.T) {
	...
	err = interloper.Put(context.Background(), chunk, mockGetAddrs)
	require.NoError(t, err)
	...
	err = store.Put(context.Background(), chunk, mockGetAddrs)
	require.NoError(t, err)
...
func TestChunkStoreCommitLocksOutFetch(t *testing.T) {
	...
	err = store.Put(context.Background(), rootChunk, mockGetAddrs)
	require.NoError(t, err)
	h, err := store.Root(context.Background())
	require.NoError(t, err)
...
func TestChunkStoreSerializeCommits(t *testing.T) {
	...
	upm.preUpdate = func() {
		...
		go func() {
			defer wg.Done()
			err := interloper.Put(context.Background(), interloperChunk, mockGetAddrs)
			require.NoError(t, err)
	...
	err = store.Put(context.Background(), storeChunk, mockGetAddrs)
	require.NoError(t, err)
	h, err := store.Root(context.Background())
	require.NoError(t, err)
