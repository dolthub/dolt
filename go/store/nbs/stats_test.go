...
func TestStats(t *testing.T) {
	...
	err = store.Put(context.Background(), c1, mockGetAddrs)
	require.NoError(t, err)
	err = store.Put(context.Background(), c2, mockGetAddrs)
	require.NoError(t, err)
	err = store.Put(context.Background(), c3, mockGetAddrs)
	require.NoError(t, err)
	...
	err = store.Put(context.Background(), c4, mockGetAddrs)
	require.NoError(t, err)
	h, err = store.Root(context.Background())
	require.NoError(t, err)
	_, err = store.Commit(context.Background(), h, h)
	...
	err = store.Put(context.Background(), c5, mockGetAddrs)
	require.NoError(t, err)
	h, err = store.Root(context.Background())
	require.NoError(t, err)
	_, err = store.Commit(context.Background(), h, h)
