...
func TestRangeDifferFromRoots(t *testing.T) {
	...
	t.Run("InterleavedEdits", func(t *testing.T) {
		// These are for the three way merge
		ctx := context.Background()
		ns := NewTestNodeStore()

		toTups, desc := AscendingUintTuplesWithStep(512, 0, 2)
		toRoot := makeTree(t, toTups)
	...
	t.Run("InterleavedInserts", func(t *testing.T) {
		ctx := context.Background()
		ns := NewTestNodeStore()
	...
	t.Run("InterleavedRemoves", func(t *testing.T) {
		ctx := context.Background()
		ns := NewTestNodeStore()
