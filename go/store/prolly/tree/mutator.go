...
// If Level > 0, then this mutation represents the range (PreviousKey, Key].
...
func ApplyMutations[K ~[]byte, O Ordering[K], S message.Serializer](
	...
	for {
		if newMutation.Level == 0 {
			...
			err = applyNodeMutation(ctx, order, chkr, cur, K(newMutation.PreviousKey), K(newMutation.Key), hash.New(newMutation.To), newMutation.SubtreeCount, newMutation.Level)
		}
		if err != nil {
			return Node{}, err
		}
...
func applyNodeMutation[K ~[]byte, O Ordering[K], S message.Serializer](
	...
	chkr *chunker[S],
	cur *cursor,
	fromKey K, toKey K, addr hash.Hash, subtree uint64, level int) (err error) {
	...
	chkr.isLeaf()
	err = insertNode(ctx, chkr, fromKey, toKey, addr, subtree, level, order)
	if err != nil {
		return err
	}
