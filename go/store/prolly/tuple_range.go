...
func (r Range) aboveStart(ctx context.Context, t val.Tuple) bool {
	...
	for i := range r.Fields {
		...
		cmp := order.CompareValues(ctx, i, field, bound.Value, typ, r.Desc.VS())
		if cmp < 0 {
...
func (r Range) Matches(ctx context.Context, t val.Tuple) bool {
	...
	for i := range r.Fields {
		...
		if r.Fields[i].BoundsAreEqual {
			...
			if order.CompareValues(ctx, i, field, v, typ, r.Desc.VS()) == 0 {
				continue
			}
			return false
		}
		...
		lo := r.Fields[i].Lo
		if lo.Binding {
			cmp := order.CompareValues(ctx, i, field, lo.Value, typ, r.Desc.VS())
		...
		hi := r.Fields[i].Hi
		if hi.Binding {
			cmp := order.CompareValues(ctx, i, field, hi.Value, typ, r.Desc.VS())
...
func closedRange(ctx context.Context, start, stop val.Tuple, desc *val.TupleDesc) (rng Range) {
	...
	for i := range rng.Fields {
		...
		isEq := order.CompareValues(ctx, i, lo, hi, desc.Types[i], desc.VS()) == 0
