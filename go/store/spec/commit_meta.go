...
func CreateCommitMetaStruct(ctx context.Context, db datas.Database, date, message string, keyValueStrings map[string]string, keyValuePaths map[string]types.Value) (types.Struct, error) {
	...
	for k, v := range keyValueStrings {
		if !types.IsValidStructFieldName(k) {
			return types.EmptyStruct(types.Format_7_18), fmt.Errorf("invalid meta key: %s", k)
		}
	...
	if date == "" {
		...
		if err != nil {
			return types.EmptyStruct(types.Format_7_18), fmt.Errorf("unable to parse date: %s, error: %s", date, err)
		}
	}
	...
	return types.NewStruct(types.Format_7_18, "Meta", metaValues), nil
}
