...
//      - else return true
func ContainCommonSupertype(f *Format, a, b *Type) bool {
	// Avoid cycles internally.
	return containCommonSupertypeImpl(f, a, b, nil, nil)
}

func containCommonSupertypeImpl(f *Format, a, b *Type, aVisited, bVisited []*Type) bool {
	if a.TargetKind() == ValueKind || b.TargetKind() == ValueKind {
		return true
	}
	if a.TargetKind() == UnionKind || b.TargetKind() == UnionKind {
		return unionsIntersect(f, a, b, aVisited, bVisited)
	}
	...
	case StructKind:
		return structsIntersect(f, a, b, aVisited, bVisited)
	case ListKind, SetKind, RefKind, TupleKind:
		return containersIntersect(f, k, a, b, aVisited, bVisited)
	case MapKind:
		return mapsIntersect(f, a, b, aVisited, bVisited)
	default:
		return true
	}
...
func unionsIntersect(f *Format, a, b *Type, aVisited, bVisited []*Type) bool {
	aTypes, bTypes := typeList(a), typeList(b)
	for _, t := range aTypes {
		for _, u := range bTypes {
			if containCommonSupertypeImpl(f, t, u, aVisited, bVisited) {
				return true
			}
		}
	}
...
func containersIntersect(f *Format, kind NomsKind, a, b *Type, aVisited, bVisited []*Type) bool {
	d.Chk.True(kind == a.Desc.Kind() && kind == b.Desc.Kind())
	return containCommonSupertypeImpl(f, a.Desc.(CompoundDesc).ElemTypes[0], b.Desc.(CompoundDesc).ElemTypes[0], aVisited, bVisited)
}

func mapsIntersect(f *Format, a, b *Type, aVisited, bVisited []*Type) bool {
	...
	return containCommonSupertypeImpl(f, aDesc.ElemTypes[1], bDesc.ElemTypes[1], aVisited, bVisited)
}

func structsIntersect(f *Format, a, b *Type, aVisited, bVisited []*Type) bool {
	...
	for i, j := 0, 0; i < len(aDesc.fields) && j < len(bDesc.fields); {
		...
		if aName < bName {
			...
			j++
		} else if !containCommonSupertypeImpl(f, aDesc.fields[i].Type, bDesc.fields[j].Type, append(aVisited, a), append(bVisited, b)) {
			i++
			j++
		} else {
			return true
