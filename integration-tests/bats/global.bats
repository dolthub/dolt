...
@test "global: basic case" {
  ...
  INSERT INTO dolt_global_tables(table_name, ref, ref_table, options) VALUES
...
@test "global: default table_name" {
  ...
  CALL DOLT_CHECKOUT('other');
  INSERT INTO dolt_global_tables(table_name, ref, options) VALUES
...
# Bikeshedding: should adding an existing table to global tables emits a warning?
@test "global: test foreign keys" {
  # Foreign keys can be added to a global table but may become invalid.

}


# Bikeshedding: should adding an existing table to global tables emits a warning?
@test "global: adding an existing table to global tables" {
  dolt checkout -b other
  run dolt sql <<SQL
  CREATE TABLE global_table (pk char(8) PRIMARY KEY);
  INSERT INTO dolt_global_tables(table_name, ref, options) VALUES
...
@test "global: detect infinite loops" {
  dolt sql <<SQL
  INSERT INTO dolt_global_tables(table_name, ref_table, options) VALUES
    ("table_alias", "table_alias", "immediate");
SQL

  run dolt sql -q "select * from table_alias;"
  [ "$status" -eq 1 ]
  [[ "$output" =~ "Table name table_alias is a nested global table. This is not allowed" ]] || false
}
