...
#    skiponwindows "Missing Dependencies"
    ...
    # Non-interactive runs should output binary as hex by default
    run dolt sql -q "SELECT b_var FROM all_bin_test WHERE id=2"
    [ "$status" -eq 0 ]
    [[ $output =~ 0xFF ]] || false

    run dolt sql -q "SELECT b_fixed FROM all_bin_test WHERE id=2"
    [ "$status" -eq 0 ]
    [[ $output =~ 0xFF00000000000000 ]] || false

    # Explicit flag
    run dolt sql -q "SELECT b_var FROM all_bin_test WHERE id=2" --binary-as-hex
    [ "$status" -eq 0 ]
    [[ $output =~ 0xFF ]] || false

    # Skip flag
    run dolt sql -q "SELECT b_var FROM all_bin_test WHERE id=2" --skip-binary-as-hex
    [ "$status" -eq 0 ]
    [[ ! $output =~ 0xFF ]] || false
    ...
    run dolt sql -r csv -q "SELECT b_var FROM all_bin_test WHERE id = 2;" --binary-as-hex
    [ "$status" -eq 0 ]
    [[ "$output" =~ "0xFF" ]] || false

    run dolt sql -r csv -q "SELECT b_var FROM all_bin_test WHERE id = 2;"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "0xFF" ]] || false
