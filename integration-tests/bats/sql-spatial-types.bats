...
@test "sql-spatial-types: create point table and insert points" {
    ...
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "insert into point_tbl () values (point(3,4)), (point(5,6))"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Query OK" ]] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "insert into point_tbl () values (point(123.456, 0.789))"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Query OK" ]] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "select st_aswkt(p) from point_tbl"
    [ "$status" -eq 0 ]
...
@test "sql-spatial-types: create linestring table and insert linestrings" {
    ...
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Query OK" ]] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "insert into line_tbl () values (linestring(point(1.2345, 678.9), point(111.222, 333.444), point(55.66, 77.88))), (linestring(point(1.1, 2.2),point(3.3, 4)))"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Query OK" ]] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "select st_aswkt(l) from line_tbl"
    [ "$status" -eq 0 ]
...
@test "sql-spatial-types: create polygon table and insert polygon" {
    ...
    [ "$status" -eq 0 ]
    [ "$output" = "" ] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "insert into poly_tbl () values (polygon(linestring(point(1,2),point(3,4),point(5,6),point(7,8))))"
    [ "$status" -eq 1 ]
    ...
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Query OK" ]] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "insert into poly_tbl () values (polygon(linestring(point(1,1),point(2,2),point(3,3),point(1,1)))), (polygon(linestring(point(0.123,0.456),point(1.22,1.33),point(1.11,0.99),point(0.123,0.456))))"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "Query OK" ]] || false
    DOLT_ENABLE_SPATIAL_TYPES=true run dolt sql -q "select st_aswkt(p) from poly_tbl"
    [ "$status" -eq 0 ]
