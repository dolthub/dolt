...
@test "all_types table has expected row count" {
    run dolt sql -q "SELECT count(*) FROM all_types;" -r csv
    [ "$status" -eq 0 ]
    [[ "${lines[1]}" =~ "3" ]] || false
}

@test "integer columns readable from old dolt" {
...
@test "negative integer columns readable from old dolt" {
...
@test "date and time columns readable from old dolt" {
    ...
    run dolt sql -q "SELECT c_time FROM all_types WHERE pk=1;" -r csv
    [ "$status" -eq 0 ]
    [[ "${lines[1]}" =~ "13:30:45" ]] || false
}

@test "datetime column readable from old dolt" {
...
@test "enum column readable from old dolt" {
...
@test "set column readable from old dolt" {
...
@test "insert row into all_types written by old dolt" {
...
@test "update all_types written by old dolt" {
...
@test "update text and blob columns to large values in old table" {
...
@test "delete from all_types written by old dolt" {
...
@test "dml round-trip: insert then read back all column types" {
...
@test "add tinytext column to old table and use dml" {
...
@test "add text column to old table and use dml" {
...
@test "add mediumtext column to old table and use dml" {
...
@test "add longtext column to old table and use dml" {
...
@test "add tinyblob column to old table and use dml" {
...
@test "add blob column to old table and use dml" {
...
@test "add mediumblob column to old table and use dml" {
...
@test "add longblob column to old table and use dml" {
...
@test "add varchar column to old table and use dml" {
...
@test "add varbinary column to old table and use dml" {
...
@test "add integer columns to old table and use dml" {
...
@test "add decimal column to old table and use dml" {
...
@test "add datetime and date columns to old table and use dml" {
...
@test "add enum column to old table and use dml" {
...
@test "add json column to old table and use dml" {
    [[ "$DOLT_VERSION" =~ 0\.50 ]] && skip "JSON type test not run for Dolt version 0.50"
...
@test "all_types schema readable from old dolt" {
...
@test "dolt diff works on all_types after dml" {
...
@test "dolt commit works after dml on all_types" {
